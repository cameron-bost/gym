...
class CarRacingPoSDiscrete(CarRacingPoSBase):
    def __init__(self, verbose=0):
    ...
    def step(self, action):
        ...
        speed_state = int(0 if speed <= 2 else math.ceil(
            speed / (MAX_SPEED / (SPEED_INTERVALS))))

        if action is not None:
            # Get raycast distances
            raycast_dist = self.get_raycast_points(close_tiles)
            raycast_dist_state = [
                int(dist // (RAY_CAST_DISTANCE / RAY_CAST_INTERVALS)) for dist in raycast_dist]
        else:
            raycast_dist_state = [RAY_CAST_INTERVALS -
                                  1 for i in range(RAY_CAST_INTERVALS)]
        # print(raycast_dist)
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            raycast_dist_normalized = 3 - \
                (3 - 1) / (75 - 10) * (sum(raycast_dist) - 10) + 1
            angle_diff_normalized = 1 - 1 / (1.2) * (angle_diff)
            steer_reward = 0.3 * (speed_reward / 2 + .1) * \
                angle_diff_normalized * raycast_dist_normalized
            ...
            try:
                next_tile = next(
                    tile for tile in close_tiles if tile.road_visited is False)
                ...
                cur_tile_dist = minimum_distance(
                    min_vertex, opp_vertex, car_pos)
            ...
            raycast_reward = -.5 + 0.5 / \
                (75 - 10) * (sum(raycast_dist) - 10) / (speed_state / 10 + 1)
...
class CarRacingPoSContinuous(CarRacingPoSBase):
    def __init__(self, verbose=0):
        super().__init__(verbose=verbose)
    ...
    def set_action_space(self):
        """
        Action Space:
        1) Steer Left: Discrete 2  - NOOP[0], Turn[1] params: min: 0, max: 1
        2) Steer Right: Discrete 2  - NOOP[0], Turn[1] params: min: 0, max: 1
        3) Gas: Discrete 2 - NOOP[0], Go[1] - params: min: 0, max: 1
        4) Brake: Discrete 2  - NOOP[0], Brake[1] - params: min: 0, max: 1
        """
        self.action_space = spaces.MultiDiscrete([3, 2, 2])
    ...
    def set_observation_space(self):
        ...
        self.observation_space = spaces.Box(low=np.array([MIN_SPEED] + [STEER_MIN] + [0.]*NUM_SENSORS),
                                            high=np.array(
                                                [MAX_SPEED] + [STEER_MAX] + [RAY_CAST_DISTANCE]*NUM_SENSORS),
                                            dtype=np.float32)
...
if __name__ == "__main__":
    ...
    env = CarRacingPoSDiscrete() #Use discrete for testing
    env.render()
